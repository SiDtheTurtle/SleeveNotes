...
- **Backend:** Python 3.12, FastAPI, SQLite, httpx, Pillow (cover thumbnail generation)
...
| POST | `/api/admin/regenerate-thumbnails` | Rebuild every `_m`/`_s` derivative from the originals on disk (non-destructive, `force=True`) |
| GET | `/api/images/manifest` | List of all derivative filenames on disk — frontend hands it to the SW to precache |
...
| `imgSrc(file, size)` | `r123_01.jpeg` → `/images/r123_01_{s\|m}.jpeg` — pick a pre-sized derivative; pair with `onerror="${imgFallback(file)}"` |
| `precacheImages()` | Fetch `/api/images/manifest`, post it to the SW to cache every derivative; called on load and after Rebuild Thumbnails |
...
3. **Data** — Import (CSV), Export (CSV, Database, Images, All), Rebuild Thumbnails (`POST /api/admin/regenerate-thumbnails`). Slow exports show "Exporting…" state on the button.
...
- **Image derivatives (FR #80):** Every cached original (`r{id}_NN.jpeg`, `m{master}_01.jpeg`) gets two smaller JPEGs generated by Pillow at download time: `_m` (400px, tiles + detail modal + carousel) and `_s` (150px, table thumbnails, sync-preview thumb). Derivatives are **always `.jpeg`** regardless of the source extension, so the frontend builds the name with a pure string transform (`imgSrc(file, size)`) — no DB lookup, no schema change. Every `<img>` carries an `onerror` fallback to the full original for legacy/missing derivatives. The lightbox still loads the original. `make_derivatives()` is idempotent (skips if the sibling exists); `backfill_derivatives()` runs once on a daemon thread at startup (`start_derivative_backfill()`, called just before the `/images` mount) and on demand via `POST /api/admin/regenerate-thumbnails` (Settings → Data → **Rebuild Thumbnails**, `force=True`). `clear_images` already deletes derivatives (they're just files in `IMAGES_DIR`); image export/import zips carry them as flat files.
...
- **PWA / service worker (`static/sw.js`):**
  ...
  - `message` `{type:'precache-images', files}`: on each page load the frontend (`precacheImages()`) fetches `/api/images/manifest` and posts it here; the SW fetches every `_m`/`_s` derivative not already cached (per-URL catch, no all-or-nothing `addAll`). Result: after one healthy load, all grid/table/modal images work offline — only the full-size lightbox needs the network. No LRU eviction; "Clear Image Cache" is the manual reset.
